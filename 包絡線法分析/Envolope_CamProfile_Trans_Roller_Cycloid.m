clear;
clc;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

syms s theta ;
rb = 40;
rf = 10;
%%rb rf;
s = theta;
%%s = sin(theta); %% any function of s

[xf1, yf1, xf2, yf2] = Envelope_roller_function(s, theta, rb, rf); %%%run the function


for i = 1:1:360
    th(i) = i/180*pi;
end
%%% XX1 = subs(xf1, theta, th);
X1 = double(subs(xf1, theta, th));
%%%YY1 = subs(yf1, theta, th);
Y1 = double(subs(yf1, theta, th));

XX2 = subs(xf2, theta, th);
X2 = double(XX2);
YY2 = subs(yf2, theta, th);
Y2= double(YY2);

figure;
hold on
plot(X1, Y1);
plot(X2, Y2);
hold off

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%@@ text book answer
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
syms M N;
r = rb + rf + s;
ds = diff(s,'theta');
M = r*sin(theta) - ds*cos(theta);
N = r*cos(theta) + ds*sin(theta);
xt = r*cos(theta) + rf/sqrt(1 + (M/N)^2);
yt = (xt*M + r*ds)/N;

XXt = subs(xt, theta, th);
Xt = double(XXt);
YYt = subs(yt, theta, th);
Yt = double(YYt);

figure;
plot(Xt, Yt);

figure;
hold on
plot(X1, Y1);
plot(X2, Y2);
plot(Xt,Yt);
hold off
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% check whether the function is right
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

syms x y;
r = rb + rf + s;
F = (x - r*cos(theta))^2 + (y - r*sin(theta))^2 - rf^2;
dF = diff(F,theta);
eqns = [F == 0, dF == 0];
S = solve(eqns, [x y]);
xcf1 = simplify(S.x(1,1));
ycf1 = simplify(S.y(1,1));
xcf2 = simplify(S.x(2,1));
ycf2 = simplify(S.y(2,1));

BBB = linspace(1,360,360);
%%%errorx = linspace(1,3600,3600);
%%%errory = linspace(1,3600,3600);
    
for i=1:360

    errorx1(i) =  X1(i)-Xt(i);
    errory1(i) =  Y1(i)-Yt(i);
    errorx2(i) =  X2(i)-Xt(i);
    errory2(i) =  Y2(i)-Yt(i);
    
end

figure;
hold on
plot(errorx1, BBB);
plot(errorx2, BBB);
hold off
figure;
hold on
plot(errory1, BBB);
plot(errory2, BBB);
hold on
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%@@ check text book answer
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% syms FF dFF;
% FF = (x - r*cos(theta))^2 + (y - r*sin(theta))^2 - rf^2;
% dFF = 2*r*sin(theta)*(x-r*cos(theta))-2*r*cos(theta)*(y-r*sin(theta));
% eqns2 = [FF == 0, dFF == 0];
% S2 = solve(eqns2, [x y]);
% xct1 = simplify(expand(S2.x(1,1)));
% yct1 = simplify(expand(S2.y(1,1)));
% xct2 = simplify(expand(S2.x(2,1)));
% yct2 = simplify(expand(S2.y(2,1)));
% 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%@@ draw the cam profile
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% t = linspace(0,2*pi,500);
% 
% xdraw = subs(x, rb, 40);
% xdraw = subs(x, rf, 10);
% xdraw = subs(x, theta, t);
% 
% ydraw = subs(y, rb, 40);
% ydraw = subs(y, rf, 10);
% ydraw = subs(y, theta, t);
% 
% figure
% plot(xdraw,ydraw);



 